    ...
    describe('#deposit', function () {
        ...
        describe('#depositTrc10()', function () {
            ...
            it('should check the TRC10 balance of mainchain and sidechain after depositTrc10', async function() {
                const tokenValue = 10;
                const dataBefore = await tronWeb.sidechain.sidechain.trx.getAccount();
                console.log("dataBefore:"+dataBefore.assetV2.toString());
        ...
        describe('#depositTrc20', function () {
            ...
            it('should check the trc20 balance after depositTrc20', async function () {
                ...
                // const contractInstance = await tronWeb.sidechain.sidechain.contract().at(ADDRESS20_MAPPING);
                //const dataBefore = await contractInstance.balanceOf(address).call();
                const account = await tronWeb.sidechain.sidechain.trx.getAccount();
                const bal = account.balance;
                console.log("bal:"+bal)
                /*let resultBefore = await tronWeb.sidechain.sidechain.transactionBuilder.triggerSmartContract(
                    ADDRESS20_MAPPING,
                    'balanceOf(address)',
                ...
                const balanceBefore = resultBefore && resultBefore.result ? new tronWeb.BigNumber(resultBefore.constant_result[0], 16).valueOf() : 0;
                ...
                let resultAfter = await tronWeb.sidechain.sidechain.transactionBuilder.triggerSmartContract(
                    ADDRESS20_MAPPING,
                    'balanceOf(address)',
                    {_isConstant: true},
                    [{ type: 'address', value: address }]
                );
                const balanceAfter = resultAfter && resultAfter.result ? new tronWeb.BigNumber(resultAfter.constant_result[0], 16).valueOf() : 0;
                // const dataAfter = await contractInstance.balanceOf(address).call();
        ...
        describe('#depositTrc721', function () {
            ...
            it('should check the trc721 balance after depositTrc721', async function () {
                ...
                // const mappingResult = await tronWeb.sidechain.mappingTrc20(HASH20, MAPPING_FEE, FEE_LIMIT,  options);
                // check the trc20 balance of mainchain before deposit
                const contractInstance = await tronWeb.contract().at(CONTRACT_ADDRESS721);
                const address = tronWeb.address.fromPrivateKey(PRIVATE_KEY);
                const dataBefore = await contractInstance.balanceOf(address).call();
                const balanceBefore = parseInt(dataBefore._hex, 16);
                console.log("balanceBefore:"+balanceBefore);
                ...
                // approve trc20
                const approveResult = await tronWeb.sidechain.approveTrc721(TRC721_ID, FEE_LIMIT, CONTRACT_ADDRESS721);
                console.log("approveResult:"+approveResult);
                await wait(20);
